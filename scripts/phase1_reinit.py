...
def build_config(
    ...
    **kwargs,
    ...
    if reinit_layers is not None:
        if isinstance(reinit_layers, str) and reinit_layers.lower() == "all":
            parsed_reinit_layers = list(range(model.n_layers))
        else:
            parsed_reinit_layers = reinit_layers
    ...
    config = ExperimentConfig(
        ...
        reinit_layers=parsed_reinit_layers,
        reinit_scale=reinit_scale,
...
def train(config: ExperimentConfig, checkpoint: Optional[str]):
    ...
    # Apply QK reinitialization if specified
    if config.reinit_first_k is not None and config.reinit_layers is not None:
        if get_local_rank() == 0:
            log.info("=" * 70)
            log.info("Applying QK reinitialization")
            log.info(f"  method: {config.reinit_method}")
            log.info(f"  first_k: {config.reinit_first_k}")
            log.info(f"  layers: {config.reinit_layers}")
            log.info(f"  init_scale: {config.reinit_scale}")
            log.info("=" * 70)

        # Get the underlying model (unwrap from distributed wrappers if needed)
        model_to_reinit = train_module.model
        if hasattr(model_to_reinit, "module"):
            model_to_reinit = model_to_reinit.module

        # Choose the appropriate reinitialization method
        if config.reinit_method == "orthogonal":
            reinit_qk_orthogonal(
        ...
        elif config.reinit_method == "zeros":
            reinit_qk_zeros(
                model=model_to_reinit,
                config=config.model,
                first_k=config.reinit_first_k,
        ...
        if get_local_rank() == 0:
            log.info("QK reinitialization complete!")
...
def main(
    *,
    ...
    USAGE = f"""
PPT Phase 1 with QK Reinitialization.

[yellow]Usage:[/] [i blue]python[/] [i cyan]{sys.argv[0]}[/] [i b magenta]launch|train|dry_run[/] [i b]RUN_NAME CLUSTER [MODEL_SIZE] [CHECKPOINT] --first_k K --layers L1 [L2 ...][/] [i][OVERRIDES...][/]
...
$ [i]python {sys.argv[0]} launch run01 ai2/jupiter-cirrascale-2 1B --first_k 4 --layers all --method zeros --launch.num_nodes=2[/]
$ [i]python {sys.argv[0]} launch run02 ai2/jupiter-cirrascale-2 --first_k 2 --layers 0 1 2 3 --method orthogonal --init_scale 0.5 --launch.num_nodes=2[/]
""".strip()
    ...
    # Parse reinitialization arguments from overrides
    reinit_first_k = None
    reinit_layers = None
    reinit_scale = 1.0
    reinit_method = "orthogonal"
    filtered_overrides = []

    i = 0
    while i < len(overrides):
        if overrides[i] == "--first_k":
            if i + 1 < len(overrides):
                reinit_first_k = int(overrides[i + 1])
                i += 2
            else:
                raise ValueError("--first_k requires a value")
        elif overrides[i] == "--layers":
            # Collect all layer indices until we hit another flag
            i += 1
            layer_list = []
            while i < len(overrides) and not overrides[i].startswith("--"):
                if overrides[i].lower() == "all":
                    layer_list = "all"
                    i += 1
                    break
                else:
                    layer_list.append(int(overrides[i]))
                    i += 1
            reinit_layers = layer_list
            ...
            if i + 1 < len(overrides):
                ...
                raise ValueError("--method requires a value")
        elif overrides[i] == "--init_scale":
            if i + 1 < len(overrides):
                reinit_scale = float(overrides[i + 1])
                i += 2
            else:
                raise ValueError("--init_scale requires a value")
        else:
            filtered_overrides.append(overrides[i])
            i += 1

    # Validate reinitialization arguments
    if reinit_first_k is None or reinit_layers is None:
        rich.get_console().print(
            "[red]Error:[/] --first_k and --layers are required for reinitialization",
            highlight=False,
        )
        rich.get_console().print(USAGE, highlight=False)
        sys.exit(1)
    ...
    if get_local_rank() == 0:
        ...
        if config.reinit_first_k is not None:
            print(
                "\n"
                f"[b yellow]QK Reinitialization:[/]\n"
                f"  method: {config.reinit_method}\n"
                f"  first_k: {config.reinit_first_k}\n"
                f"  layers: {config.reinit_layers}\n"
                f"  init_scale: {config.reinit_scale}"
            )
